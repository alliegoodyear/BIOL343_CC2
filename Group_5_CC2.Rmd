...
positpal::start("Group_5_CC2.Rmd")
```
...
1. **Coralie Spakowski** - 20471806 + [BIOL343_CC] (https://github.com/coraliespakowski/BIOL343_CC)
1. **Jake Mayo** - 20186062 + [BIOL_343.CC] (https://github.com/jake-mayo/BIOL343.CC)
1. **Noah Lu** - 20483156 + [BIOL_343.CC] (https://github.com/jake-mayo/BIOL343.CC)
1. **Iryna Ozimek** - 20508541 + [BIOL_343.CC] (https://github.com/irynaoz323/BIOL343.CC)
...
---------|-------------|---------------------------------
population_type | asexual or sexual poppulation type| NA
ovary_width | plant ovary width | mm |
ovary_height | plant ovary height | mm |
flower_number | plant flower number | count |
...
$The\:Units\:of\:Ovary\:Area\:are\: mm^2$
...
#area_means #checking the values
...
```{r area-table, echo=F, warning =F, message =F}
oatable = area_means
library(knitr)
kable(oatable, caption = "Ovary Area by Population Type of Solanum pimpinellifolium")
```
...
library(ggplot2)
vizualization = ggplot(aes(x=ovary_area, fill = population_type), data =cc1_data) + 
    geom_histogram(binwidth = 1) + 
    theme_classic() +
    xlab("Ovary Area (mm^2)") +
    ylab("Count") +
    labs(fill = NULL) +
    scale_fill_discrete(labels = c("Asexual", "Sexual")) +
    labs(title = "Ovary Area by Population Type in Solanum pimpinellifolium")
vizualization + facet_wrap(.~population_type)
```
